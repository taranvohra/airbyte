...
3. `SECRET_PERSISTENCE` - Defines the Secret Persistence type. Defaults to NONE. Set to GOOGLE\_SECRET\_MANAGER to use Google Secret Manager. Set to TESTING\_CONFIG\_DB\_TABLE to use the database as a test. Alpha support. Undefined behavior will result if this is turned on and then off.
