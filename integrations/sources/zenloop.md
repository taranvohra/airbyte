...
The `Answers` and `AnswersSurveyGroup` stream respectively have an optional survey\_id parameter that can be set by filling the `public_hash_id` field of the connector configuration. If not provided answers for all surveys (groups) will be pulled.
